...
class Store_controller extends Base_Authenticated_Controller {
     ...
     **/
          ...
          public function index()
     {
        ...
        $this->breadcrumbs->push('Stores', '/store');
         ...
         function update($id)
    {
        ...
        $this->breadcrumbs->push('Administrator', '/');
        $this->breadcrumbs->push('Stores', '/store');
         $this->breadcrumbs->push('Update', '/store/update');
         ...
         $this->template->build(config_item('current_theme_path').'stores_update' , $stores);
         ...
         public function create() 
    {
          ...
          $this->breadcrumbs->push('Administrator', '/');
          $this->breadcrumbs->push('Stores', '/store');
          $this->breadcrumbs->push('Add', '/store/add');
 ...
 /**
    ...
    /**
     * @author: Jasdeep Singh <jasdeep.singh@relesol.com>
     * @purpose: Upload the image for logo   of store
     * @created_at: Sept 30, 2015 
     * @library: 
     * @models:   
     * @helpers:   
     * @3rdparty:     
    ...
    {
        $upload_path_url = $this->config->item('base_path') . 'upload/stores_logo/';
...
exit;*/
        ...
        $_FILES['files']['size']= $_FILES['files']['size'][0];
            ...
            $this->upload->initialize($config);
              ...
              if ($this->upload->do_upload('files'))
              {
                  ...
                  $this->load->model('Stores');
            $update_store_logo_data = array(
            'id' => $store_id,
            'logo_img' => $file_data['upload_data']['orig_name']                    
         );
                  $store_id = $this->Stores->updateLogoImage($update_store_logo_data);     
                  return  $store_id;
              } 
               else
              {
    ...
    /**
     * @author: Jasdeep Singh <jasdeep.singh@relesol.com>
     * @purpose: Upload the image for cover of cover
     * @created_at: Sept 30, 2015 
     * @library: 
     * @models:    
     * @helpers:   
    ...
    {
        $upload_path_url = $this->config->item('base_path') . 'upload/stores_cover_image/';
        ...
        $this->load->library('upload', $config);
        $store_id = $this->input->post('stores_id');
        $images =  time();
/*
echo "<pre>";
print_r($_FILES['files']['name']);
exit;*/
        ...
        $_FILES['files']['size']= $_FILES['files']['size'][0];
            ...
            $update_store_logo_data = array(
            'id' => $store_id,
            'cover_img' => $file_data['upload_data']['orig_name']                    
         );
                  $store_id = $this->Stores->updateCoverImage($update_store_logo_data);     
                  return  $store_id;
              }
               else
              {
...
public function activeStatus($id)
{
   ...
   redirect('store/index');   
         ...
         function update_store($id)
    {
         ...
         $stores['document_status_cover'] = $this->Stores->getStoreCoverStatus($id);
         $stores ['users'] = $this->Users->get_all_users();
         $this->template->build(config_item('current_theme_path').'stores_account' , $stores);
