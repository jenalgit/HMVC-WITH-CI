		...
		try
		{
			...
			if ($this->hybridauthlib->providerEnabled($provider))
			{
				...
				if ($service->isUserConnected())
				{
					...
					$check_sql=$this->db->query("select * from social_users where identifier='".$user_profile->identifier."' limit 1")->result_array();

							if(count($check_sql)>0){

								$check_sql=$check_sql[0];
								$social_id=$check_sql['id'];

								$identifier=$user_profile->identifier;
								
								if($provider=='Twitter'){

										$photoURL=str_replace('_normal','',$user_profile->photoURL);

										$array_url=explode('/',$photoURL);
										$array_url=array_reverse($array_url);

										
										$avatarkey = $array_url[0];
								...
								'professional_skills'	=>$user_profile->description,
		...
		if ($this->form_validation->run() == true)
		{
			...
			if(count($result_sql)>0){
				...
				if($provider=='Twitter'){
						...
						$res_user=$this->db->query("select * from users where email_id='".$this->input->post('email')."' limit 1")->row_array();
                            ...
                            if(count($res_user)>0){
                                $user_data = array(
										...
										'professional_skills'	=>$this->input->post('description'),
									     'verified_at'			=>'1',
				                         'active'				=>'1',  
				                         'grp_fk_id'			=>'3',
				                         'photo'				=>$photo_image,
