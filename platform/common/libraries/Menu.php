 ...
 * @license         Note by Deepak Patil <deepak.patil@relesol.com>, 01-JAN-2014: According to the provided links
...
class Menu {
    ...
    public function normalize_items($items)
    {
        ...
        if (is_array($items))
        {
            ...
            foreach($items as $key => $val)
            {
                ...
                // Modified by Deepak Patil <deepak.patil@relesol.com>, 04-JAN-2014.
    ...
    protected function _render_array($menu, $level = 0)
    {
        ...
        if (!empty($menu) AND (isset($this->depth) AND $level < $this->depth) OR !isset($this->depth))
        {
            foreach($menu as $key => $val)
            {
                // Added by Deepak Patil <deepak.patil@relesol.com>, 04-DEC-2014.
                ...
                $subitems = $this->_get_menu_items($val['id']);
                // Modified by Deepak Patil <deepak.patil@relesol.com>, 22-MAR-2014.
                // A dirty workaround.
    ...
    protected function _get_label($label)
    {
        ...
        if (is_array($label))
        {
            // Modified by Deepak Patil <deepak.patil@relesol.com>, 04-DEC-2013.
            //$label = $label['label'];
